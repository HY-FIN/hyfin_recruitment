/**
 * Gmail 발송용 리프레시 토큰 발급 헬퍼 (로컬 1회 실행용)
 *
 * 실행법:
 *   GOOGLE_OAUTH_CLIENT_ID=... GOOGLE_OAUTH_CLIENT_SECRET=... npx tsx scripts/get-gmail-oauth-token.ts
 *
 * 절차:
 *   1. 출력된 URL 을 브라우저에서 열고 hyu.hyfin@gmail.com 계정으로 로그인/동의
 *   2. 동의 후 localhost:53682 로 리다이렉트되면 콘솔에 refresh_token 이 출력됨
 *   3. 출력된 값을 GMAIL_OAUTH_REFRESH_TOKEN 환경변수로 저장
 */
...
async function main() {
  ...
  if (!clientId || !clientSecret) {
    ...
    console.error(
      "예: GOOGLE_OAUTH_CLIENT_ID=... GOOGLE_OAUTH_CLIENT_SECRET=... npx tsx scripts/get-gmail-oauth-token.ts"
    );
    process.exit(1);
  }
  ...
  console.log(
    "\n브라우저에서 이 URL을 열고 hyu.hyfin 계정으로 로그인/동의하세요. '확인되지 않은 앱' 경고가 나오면 고급 → 계속(안전하지 않음)으로 진행하세요:\n"
  );
  console.log(authUrl);
